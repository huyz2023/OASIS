...
class challengeDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {"image": image, "label": label, "name": self.images[idx], "label_name": self.labels[idx]}
        

    def list_images(self):
        mode = "val" if self.opt.phase == "test" or self.for_metrics else "train"
        path_img = os.path.join(self.opt.dataroot, "imgs") #TODO:修改成正确路径
        path_lab = os.path.join(self.opt.dataroot, mode+"_labels")
        path_lab_to_img = os.path.join(self.opt.dataroot,"label_to_img.json")
        if mode == 'train':
          images = sorted(os.listdir(path_img)) 
          labels = sorted(os.listdir(path_lab))
        else:
          with open(path_lab_to_img,"r") as f:
            lab_to_img = json.load(f)
          labels = list(lab_to_img.keys())
          images = [lab_to_img[label] for label in labels]
